...
bool checkBomb(int K) {
    ...
    rep(i, 0, N) {
        ...
        // Two intersections - find the enclosed area
        // Is 0 in the enclosed area?
        Point dir = walkInDir(plane, getVectorInDir(basis, 0), R);
        bool isNormal = true;
        if (sphericalDist(A[i], dir) <= R) {
            // Yes, need to reverse the search (find non-enclosed area)
            isNormal = false;
        }
        // First, find location where it first goes into the area
        double low = 0, high = 2*PI;
        while (high-low > 1e-9) {
            double mid = (low+high)/2;
            Point dir = walkInDir(plane, getVectorInDir(basis, mid), R);
            // assert(abs(sphericalDist(A[K], dir)-R) < 1e-9);
            if ((sphericalDist(A[i], dir) <= R) == isNormal) high = mid;
            else low = mid;
        }
        double firstTime = (low+high)/2;
        // Also find the last time
        low = 0, high = 2*PI;
        while (high-low > 1e-9) {
            double mid = (low+high)/2;
            Point dir = walkInDir(plane, getVectorInDir(basis, mid), R);
            if ((sphericalDist(A[i], dir) <= R) == isNormal) low = mid;
            else high = mid;
        }
        double lastTime = (low+high)/2;

        // Record the intervals
        if (isNormal) {
            events.push_back({firstTime, true});
            events.push_back({lastTime, true});
        } else {
            events.push_back({0, true});
            events.push_back({firstTime, false});
            events.push_back({lastTime, true});
        }
        cout << "For " << K << " against " << i << ": " << isNormal << " " << firstTime << " " << lastTime << endl;

        Point directPath = A[i] - A[K];
        double distToPlane = plane.dot(directPath); // - dPlane;
        Point dirToBomb = directPath - (plane * distToPlane);
