...
public class pieatersnew {
    int N, M;
    int[][] maxWeight;
    final long INF = 9988776655443322L;
    // bestCovered[i][j][k] = Best segment covering k, within bounds i and j
    int[][][] bestCovered;

    pieatersnew() throws IOException {
        ...
        // Generate bestCovered
        bestCovered = new int[N][N][N];
        for (int i = 0; i < N; i++) bestCovered[i][i][i] = maxWeight[i][i];
        for (int len = 1; len < N; len++) {
            for (int left = 0; left < N; left++) {
                if (left + len >= N) break;
                int right = left + len;
                for (int k = left; k <= right; k++) {
                    bestCovered[left][right][k] = Math.max(bestCovered[left+1][right][k], bestCovered[left][right-1][k]);
                    // Try new segment also
                    bestCovered[left][right][k] = Math.max(maxWeight[left][right], bestCovered[left][right][k]);
                }
            }
        }
        ...
        for (int len = 1; len < N; len++) {
            for (int left = 0; left < N; left++) {
                ...
                for (int j = left + 1; j < right; j++) {
                    ...
                    bestSeg = bestCovered[left][right][j];
                    dpMiddle += bestSeg;
