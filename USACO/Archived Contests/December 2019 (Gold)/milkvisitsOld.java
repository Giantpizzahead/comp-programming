...
public class milkvisitsOld {
    ...
    milkvisitsOld() throws IOException {
    ...
    HashMap<Integer, ArrayList<Query>> dfsQueries(int n, int p) {
        // First process leaves
        ArrayList<HashMap<Integer, ArrayList<Query>>> leafQueries = new ArrayList<>(2);
        ...
        HashMap<Integer, ArrayList<Query>> merged;
        if (isLeaf) {
            // Just add an empty set for easier implementation
            leafQueries.add(new HashMap<>(Math.min(subtreeSize[n], 11)));
            maxQueryEntries = 0;
            maxQueryI = 0;
            ...
            if (merged.containsKey(type[n])) {
                for (Query q : merged.get(type[n])) {
                    if (q.isValid) answers[q.origI] = true;
                    maxQueryEntries--;
                }
                merged.remove(type[n]);
            }
        }
        ...
        HashMap<Integer, ArrayList<Query>> toMerge;
