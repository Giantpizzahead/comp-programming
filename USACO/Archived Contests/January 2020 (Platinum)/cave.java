...
public class cave {
    ...
    void evalComponent(Component comp) {
        ...
        DisjointSet ds = new DisjointSet(dsSize);
        int actualX = -1, lastY = -1;
        for (Point p : comp.points) {
            if (lastY != p.y) {
                // Moved up a row; add 1 to all current sets
                for (int i = 0; i < ds.size; i++) ds.numWays[i]++;
            }
            actualX = p.x - comp.minX;
            // Merge with sides
            ds.setActive(actualX);
            if (actualX != 0) ds.union(actualX - 1, actualX);
            if (actualX != ds.size - 1) ds.union(actualX, actualX + 1);
            lastY = p.y;
        }

        // Off-by-one (top row filled)
        // Get final answer
        long compWays = ds.numWays[ds.find(actualX)] + 1;
