...
public class valleys4 {
    int N;
    int[][] grid;
    Point[] points;
    ArrayList<Trigger> holeyTriggers;

    valleys4() throws IOException {
    ...
    void findMountains() {
        ...
        for (int i = points.length - 1; i >= 0; i--) {
            ...
            if (cWillJoinEdge) {
                ...
                // If they are, make a new holey trigger with value = max value in the most recently updated neighbor
                // (the one with the smallest size)
                int lowestSize = 999999999;
                HashSet<Integer> uniqueNeighbors = new HashSet<>(4);
                for (int neighbor : neighbors) {
                    if (neighbor != -1) {
                        uniqueNeighbors.add(neighbor);
                        if (ds.getSize(neighbor) < lowestSize) {
                            // Update blocked value
                            maxBlocked = maxValueInSet[neighbor];
                            lowestSize = ds.getSize(neighbor);
                        }
                    }
                }
            ...
            for (int neighbor : neighbors) {
                ...
                minValueInSet[newSet] = Math.min(minValueInSet[neighbor], minValueInSet[oldSet]);
