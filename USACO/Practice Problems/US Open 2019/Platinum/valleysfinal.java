...
public class valleysfinal {
    int N;
    int[][] grid;
    Point[] points;
    ArrayList<Trigger> holeyTriggers;

    valleysfinal() throws IOException {
    ...
    void findMountains() {
        ...
        for (int i = points.length - 1; i >= 0; i--) {
            ...
            // If joined to the edge, make a new holey trigger with value = max of all neighbors that aren't
            // already joined to the edge
            int maxBlocked = -1;
            if (cWillJoinEdge) {
                for (int neighbor : neighbors) {
                    if (neighbor != -1 && !setIsOnEdge[neighbor]) {
                        // This neighbor is in a mountain that isn't joined to the edge; add its max value to trigger
                       maxBlocked = Math.max(maxValueInSet[neighbor], maxBlocked);
                    }
                }
            } else {
                // Check if two inner mountains are being joined
                // If they are, make a new holey trigger with value = max value in the inside neighbor
                // Use minX of each mountain to find the inside one (minX of inside > minX of outside
                // Assume that one mountain is in the other one; if this isn't true, it will fix itself once it joins
                // with the edge
                int lowestX = -999999999;
                HashSet<Integer> uniqueNeighbors = new HashSet<>(4);
                for (int neighbor : neighbors) {
                    if (neighbor != -1) {
                        uniqueNeighbors.add(neighbor);
                        if (minXInSet[neighbor] > lowestX) {
                            // Assume inside mountain; update blocked value
                            maxBlocked = maxValueInSet[neighbor];
                            lowestX = minXInSet[neighbor];
                        }
                    }
                }

                // If two or more mountains being joined, keep the holey trigger
                if (uniqueNeighbors.size() < 2) maxBlocked = -1;
            }
            ...
            for (int neighbor : neighbors) {
                ...
                minXInSet[newSet] = Math.min(minXInSet[neighbor], minXInSet[oldSet]);
