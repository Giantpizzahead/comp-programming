...
Optimization: Do the DFS recursively instead of iteratively.
...
public class pushaboxopt {
    ...
    pushaboxopt(BufferedReader in, PrintWriter out) throws IOException {
...
//        for (int y = 0; y < rows; y++) System.out.println(Arrays.toString(comp[y]));
    ...
    void mainSearch(int x, int y, int prevComp, int d) {
        ...
        for (int i = 0; i < 4; i++) {
            if (i == d) {
                ...
                if (!visited[my][mx][i] && grid[my][mx] != '#') {
                    ...
                    mainSearch(mx, my, comp[y][x], i);
                }
            } else {
                nx = x + cx[i];
                ny = y + cy[i];
                if (!visited[y][x][i] && prevComp == comp[ny][nx]) {
                    // In same component, so Bessie can reach it
                    visited[y][x][i] = true;
                    mainSearch(x, y, comp[ny][nx], i);
                }
            }
        }
    }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!singleVisited[ny][nx] && grid[ny][nx] != '#') {
                if (grid[ny][nx] == 'B') {
                    ...
                    mainSearch(nx, ny, comp[y][x], (i + 2) % 4);
