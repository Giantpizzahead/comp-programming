...
public class fairphoto {
    ...
    fairphoto(BufferedReader fin, PrintWriter fout) throws IOException {
    ...
    class DeltaTreeSet {
        ...
        void transitionInto(DeltaTreeSet newDTS) {
            for (State state : ts) {
                // Get raw counts of cows
                int[] rawCounts = new int[8];
                int actualRootCount = state.rootCount - rootZero;
                for (int i = 0; i < 8; i++) {
                    if (i == rootBreed) rawCounts[i] = actualRootCount;
                    else if (breedToI[i] == -1) continue;
                    else rawCounts[i] = actualRootCount + (state.relBreeds[breedToI[i]] - relZero[breedToI[i]]);
                }
                // System.out.println(Arrays.toString(rawCounts));

                // Figure out new deltas
                int[] newRelBreeds = new int[numBreeds];
                int rootDelta = rawCounts[newDTS.rootBreed];
                for (int i = 0; i < numBreeds; i++) {
                    int targetBreed = newDTS.iToBreed[i];
                    newRelBreeds[i] = rawCounts[targetBreed] - rootDelta + newDTS.relZero[i];
                }
