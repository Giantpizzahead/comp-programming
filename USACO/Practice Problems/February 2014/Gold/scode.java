/**
 * USACO 2014 February Contest (Silver -> Gold)
 * Problem 3. Secret Code
 * 
 * This is for USACO practice (and to get the one timed out case).
 */
...
public class scode {
	...
	scode() throws IOException {
		...
		for (int i = startingString.length(); i >= 2; i--) {
			if (VISUAL_DEBUG) {
				System.out.print(strings[i].size() + " ");
				if (i % 10 == 0) {
					System.out.println();
				}
			}
			...
			if (l != 2) {
				...
				for (int i = 0; i < (strlen - 1) / 2; i++) {
					...
					if (string.substring(0, i+1).equals(string.substring(i+1, (i+1) * 2))) {
						...
						if (newNumWays >= 2014) newNumWays -= 2014;
