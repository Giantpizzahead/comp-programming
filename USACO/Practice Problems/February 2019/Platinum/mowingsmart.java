...
public class mowingsmart {
    ...
    mowingsmart() throws IOException {
    ...
    long sweepFlowers() {
        ...
        SegmentTree stNumFlowers = new SegmentTree(N+1);
        val = new long[N];
        for (int i = 0; i < N; i++) val[i] = INF;
        val[0] = 0;
        TreeSet<Point>[] treeSets = new TreeSet[N];

        // Treat (0, 0) as a base case
        Comparator<Point> comparingYs = Comparator.comparingInt(a -> a.y);
        treeSets[0] = new TreeSet<>(comparingYs);
        treeSets[0].add(new Point(0, 0, 0));

        Point curr, edge1, edge2;
        for (int i = 1; i < N; i++) {
            curr = flowers[i];
            ...
            // Try all possible flowers
            for (Point p : treeSets[numFlowers].headSet(curr)) {
                val[i] = Math.min(val[p.i] + calcRect(curr, p), val[i]);
            }
            // Add new numFlowers to segment tree
            stNumFlowers.update(yti.get(curr.y), numFlowers + 1);
            // Add point to both TreeSets
            if (treeSets[numFlowers+1] == null) {
                treeSets[numFlowers+1] = new TreeSet<>(comparingYs);
            }
            treeSets[numFlowers+1].add(new Point(curr.x, curr.y, i));
        }
